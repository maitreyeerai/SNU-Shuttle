		...
		<div class="col-md-12">
			...
			<div class="form-group mb-2">
				...
				<select name="to_location" id="to_location" class="form-control" required>
					...
					<?php while($row = $location->fetch_assoc()){ ?>
						<option value="<?php echo $row['id'] ?>" <?php echo isset($meta['to_location']) && $meta['to_location'] == $row['id'] ? 'selected' : ''  ?>><?php echo $row['location']  ?></option>
					<?php } ?>
				</select>
			</div>
			<div class="form-group mb-2">
			...
			<div class="form-group mb-2">
				<label for="eta" class="control-label">Return Time</label>
