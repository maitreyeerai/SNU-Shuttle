...
if((isset($_SESSION['login_username']) and $_SESSION['login_user_type']==1) && isset($_GET['bid'])){
...
<div class="container-fluid">
	<form id="manage_book">
		<div class="col-md-12">
			...
			<p><b>Departure:</b> <?php echo date('M d,Y h:i A',strtotime($meta['departure_time'])) ?></p>
			<p><b>Return:</b> <?php echo date('M d,Y h:i A',strtotime($meta['eta'])) ?></p>
			<?php if(($count < $meta['availability']) || (isset($_SESSION['login_username']) and $_SESSION['login_user_type']==1)): ?>
			...
			<div><b>Name:</b> <?php echo (isset($_SESSION['login_username']) and $_SESSION['login_user_type']==1) ? $bmeta['name'] : $_SESSION['login_name']?></div>
			<br>
			...
			<div class="form-group mb-2">
				...
				<select  class="form-control" id="status" name="status" value="<?php echo isset($bmeta['status']) ? $bmeta['status'] : '' ?>">
		...
		$.ajax({
			...
			success:function(resp){
				...
				if(resp.status == 1){
    				...
    				if('<?php echo !(isset($_SESSION['login_username']) and $_SESSION['login_user_type']==1) ?>' == 1){
    				$('#book_modal .modal-body').html('<div class="text-center"><p><strong><h3>'+resp.ref+'</h3></strong></p><small>Reference Number</small><br/><small>Send your reference no. while paying online at 9XXXX </small></div>')
